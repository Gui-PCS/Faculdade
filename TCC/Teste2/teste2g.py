...
# Chave da API Google Maps (obtenha sua chave em https://cloud.google.com/maps-platform/)
chave_google_maps = "AIzaSyCEEqRHTgHUYVHBooQCw8t2Uw5A3h43toA"

# Crie um cliente da API do Google Maps
gmaps = Client(key=chave_google_maps)

# Função para calcular a distância entre dois pontos
def distancia(ponto1, ponto2):
    lat1, lon1 = ponto1
    lat2, lon2 = ponto2
    cache_file = f"distancesg/{lat1}_{lon1}_{lat2}_{lon2}.json"
    if os.path.exists(cache_file):
        ...
        if datetime.now() - mod_date > timedelta(days=20):
            ...
            with open(cache_file, 'r') as f:
                distancia = json.load(f)['rows'][0]['elements'][0]['distance']['value']
                return distancia

    # Use o cliente da API do Google Maps para calcular a distância
    matriz_distancias = gmaps.distance_matrix(origins=ponto1, destinations=ponto2, mode="driving")
    distancia = matriz_distancias['rows'][0]['elements'][0]['distance']['value']
    with open(cache_file, 'w') as f:
        json.dump(matriz_distancias, f)
    return distancia
...
for i in range(len(rota_otimizada) - 1):
    ...
    # Chamada à API Google Maps para obter instruções de rota
    direcoes_resultado = gmaps.directions(origem, destino, mode="driving")
    # Extração das instruções de rota
    for passo in direcoes_resultado[0]['legs'][0]['steps']:
        instrucoes_rota.append(passo['html_instructions'])
